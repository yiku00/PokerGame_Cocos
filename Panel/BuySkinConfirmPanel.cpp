...
BuySkinConfirmPanel::BuySkinConfirmPanel()
{
	initPanel();
}
...
void BuySkinConfirmPanel::initPanel()
{
	addMouseBlockPanel(this , true);	//ok 처리
	construct(getMxmlPath() + "dmy_get_board_buycard.mxml");	
	setButtonListener(this);
	mTitle = getLabel("txt_title");
	mTitleUnder = getLabel("txt_title_under");
	//titie
	mTitle->setString(GameStringDepot::getSingleton().getString("SKIN_CONFIRM_TITLE").c_str());
	mTitleUnder->setString(GameStringDepot::getSingleton().getString("SKIN_CONFIRM_TITLE").c_str());

	mCharacterImg = getImage("dmy_card");
...
void BuySkinConfirmPanel::setCharacterConfirm(int characterId)
{
	mCharacterShopId = characterId;
	W_SkinShopData _ShopInfo = GameDataManager::getSingletonPtr()->GetGameDataPtr()->skinshopdata(characterId);
	mCharacterId = _ShopInfo.skinid();
	const W_SkinData* skinData = GameDataManager::getSingleton().getSkinData(mCharacterId);
	string characterNameStr = "";
	string itemDescStr = "";
	int characterImgId = -1;
	if (skinData->has_id() && _ShopInfo.has_id())
	{
		if (skinData->id() == _ShopInfo.id())
		{
			characterImgId = skinData->id();
			if (skinData->has_name())
				characterNameStr = skinData->name();
		...
		//금액 표시		
		mEventStr = "Card_Shop_Res_" + StringConverter::toString(characterId);
...
void BuySkinConfirmPanel::onClicked(const string& name)
{
	if (name == "buy") {
		if (isCharacterMoneyType()) {
			sendPayReserveReq(mCharacterShopId);
		}
		else {
...
bool BuySkinConfirmPanel::isCharacterMoneyType()
{
	bool isMoneyType = false;	
	W_SkinShopData _infoData = GameDataManager::getSingletonPtr()->GetGameDataPtr()->skinshopdata(mCharacterShopId);
	if (_infoData.cash() == 0) {
		isMoneyType = true;
	}	
	return isMoneyType;
...
void BuySkinConfirmPanel::sendPayReserveReq(int cashId)
{
	mBuyCashId = cashId;
	W_PayReserveReq req;
#if (CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID) 
	...
	req.set_id(cashId);
	HttpMessage msg(req, HttpResponseHandler(BuySkinConfirmPanel::recvWPayReserveRes), this);
	HttpManager::getSingleton().send(msg);
}

void BuySkinConfirmPanel::recvWPayReserveRes(HttpMessage* msg)
{
	if (msg->hasCode()) {
		return;
	}
...
bool BuySkinConfirmPanel::handling(STCMD &stCmd)
{
	if (!isVisible()) {
...
void BuySkinConfirmPanel::sendW_PayReq(string tID, string txId, string receipt)
{
	W_PayReq req;
	req.set_tid(tID);
	req.set_txid(txId);
...
#endif
	HttpMessage msg(req, HttpResponseHandler(BuySkinConfirmPanel::recvW_PayRes), this);
	HttpManager::getSingleton().send(msg);
}

void BuySkinConfirmPanel::recvW_PayRes(HttpMessage* msg)
{
	if (msg->hasCode()) {
		return;
	}
