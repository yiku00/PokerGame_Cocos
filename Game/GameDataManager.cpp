...
int GameDataManager::getModelInfo(int id , ModelInfo val)
{
	...
	if (GameDataManager::getSingletonPtr()->ModelInvenTableData.size() <= 0)
		return -20;

	int tabid = -1;
	int startN = -1;
	for (int i = 0; i < ModelInvenTableData.size(); i++)
	{
		if (ModelInvenTableData[i].startn() <= id && id <= ModelInvenTableData[i].endn()) {
			tabid = ModelInvenTableData[i].id();
			startN = ModelInvenTableData[i].startn();
			break;
		}
	}
